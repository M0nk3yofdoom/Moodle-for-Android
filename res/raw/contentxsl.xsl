...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="node()">
        ...
        <xsl:for-each select="KEY">
			...
			<xsl:if test="VALUE/node() != '' and @name != 'modules'">
				<xsl:text>"</xsl:text>
			...
			<xsl:if test="(not(VALUE/node()) or VALUE/node() = '') and @name != 'modules'">
				<xsl:choose>
					<xsl:when test="position() = last()">
						<xsl:text>" " </xsl:text>
					</xsl:when>
			...
			<xsl:if test="@name = 'modules'">
				...
				<xsl:for-each select="MULTIPLE/SINGLE">
					...
					<xsl:for-each select="KEY">
						...
						<xsl:if test="@name != 'contents' and VALUE/node() != ''">
							<xsl:text>"</xsl:text>
							<xsl:value-of select="VALUE/node()"/>
							<xsl:choose>
								<xsl:when test="position() = last()">
						...
						<xsl:if test="@name != 'contents' and (not(VALUE/node()) or VALUE/node() = '')">
							<xsl:choose>
								<xsl:when test="position() = last()">
									<xsl:text>" " </xsl:text>
								</xsl:when>
